...
var hackerearth_url="https://www.hackerearth.com/users//";

var github_url="https://github.com/Kashish-111";
var instagram_url="https://www.instagram.com/kashish_tyagiii__/6";

var resume_url="https://github.com/Kashish-111/My-resume/blob/main/Purple%20and%20White%20Clean%20and%20Professional%20Resume.pdf";
...
function preload(){
	...
	game.load.image('ambulace','assets/ambulance2.png');
	...
	game.load.image('instagram','assets/fb.png');
	...
	game.load.image('blogLoo','assets/icon_small.png');
...
//	game.load.onLoadStart.add(loadStart, this);
  ...
  //  game.stage.scale.startFullScreen();
   ...
   //Add Explosion object pool
	...
	var banner = game.add.sprite(250,h-200-425,'mainbanner').scale.setTo(1.4,1.4);
	...
	game.add.sprite(base5,h-440,'level');
	game.add.text(base5+70,h-340,"Level 5");
	game.add.text(base5+135,h-260,"Online Profiles",{font:"20px Arial Black"});
	...
	game.add.sprite(13400,h-360,'harddrive').scale.setTo(0.6,0.6);
	game.add.sprite(13400,h-360,'bits');
	game.add.text(13400,h-400,"Slot-machine\n using Python .",style_white);

	var pd = game.add.sprite(14050-100,h-160,'');
	pd.scale.setTo(1.4,1.4);
	pd.angle = 90;
	game.add.text(13800-100,h-300,"",style_white);

	game.add.sprite(14070,h-600,'hospital');
	...
	t.start();
	game.add.text(14240,h-370,"Factory managment System \n DBMS project.",style_white);
	...
	shakti_cup = game.add.sprite(base5+500,-500,'');
	shakti_cup.scale.setTo(0.5,0.5);
	game.add.text(base5+505,h-300," ",style_white);
	...
	cup1 = game.add.sprite(base5+820+200,-280,'');
	...
	game.add.text(base5+2080+50,h-310,"",style_position);

	game.add.text(base5+2060,h-305+50," ",style_position);
	game.add.sprite(base5+2170,h-310+50,'');
	game.add.text(base5+2060,h-205+30,"",style_position);
	...
	//Add Contact Buttons
	game.add.sprite(base5+600,h-400,'ladder');
	
	var cloud_blog =game.add.sprite(base5+550,h-700+20,'');	
	...
	game.add.sprite(base5+400,h-400,'cloud').scale.setTo(1.2,1.2);
	button_github = game.add.button(base5+430,h-330,'github',openGithub, this);
	button_github.scale.setTo(0.20,0.20);

	
	
	game.add.sprite(base5+800,h-550,'cloud').alpha=0.8;
	button_fb = game.add.button(base5+860,h-520,'instagram',openinstagram,this).scale.setTo(0.8,0.8);
	
	
	
	
	var gmail_cloud=game.add.sprite(base5+670,h-440,'cloud');
	gmail_cloud.scale.setTo(1.1,1.1);
	gmail_cloud.alpha =0.95;
	button_gmail = game.add.button(base5+720,h-400,'gmail',openGmail, this);
	...
	var linkedin_cloud = game.add.sprite(base5+1050-100,h-530,'clod')
	linkedin_cloud.scale.setTo(1.2,1.2);
	linkedin_cloud.alpha =0.9;
	button_linkedin = game.add.button(base5+990,h-450,'linkedi',openLinkedIn, this);
	button_linkedin.scale.setTo(0.8,0.8);
	
	game.add.text(base5+720,h-120,"Click /Tap the Clouds to open Profiles .",style_white);
	
	game.add.button(base5+1260,h-450+20,'',openPublication,this);
	...
	*/
	//Add hometown
	var hometown = game.add.sprite(1600,h-460,'hometown').scale.setTo(0.75,0.75);;
	...
	aieee_cup=game.add.sprite(3500,-500,'');
	game.add.text(4300,h-120,"  .",style_roboto);
	
	//Add bubble
	
	bubble = game.add.sprite(9100,h+450,'');
	game.add.text(8900,h+500,"Designing Skills - canva",style_roboto);
...
*/
	...
	//Add School
	var school = game.add.sprite(2200,h-220-219,'school');
	//school.scale.setTo(1.2,1.2);

	var clg = game.add.sprite(4000,h-220-419,'dtu');
	clg.scale.setTo(1.1,1.1);
...
function update(){
	...
	if(hero.x > base7+800 && hero.x <=base5+900 && !cupTweenDone){
...
function openinstagram(){
 var win = window.open(instagram_url);
 win.focus();
}
